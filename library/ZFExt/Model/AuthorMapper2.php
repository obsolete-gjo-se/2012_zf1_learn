...
class ZFExt_Model_AuthorMapper extends ZFExt_Model_Mapper
{
    ...
    public function find ($id)
    {
        ...
        $result = $this->_getGateway()
            ->find($id)
            ->current();
        $author = new $this->_entityClass(
                array('id' => $result->id, 'fullname' => $result->fullname, 
                        'username' => $result->username, 
                        'email' => $result->email, 'url' => $result->url));
